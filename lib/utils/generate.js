 ...
 * @param suffix 域名后缀
 */
export default async function getAvailableDomains(
  pinyinCount,
  maxLength,
  suffix = '.com',
) {
  ...
  // 创建写入流
  const out = fs.createWriteStream(`domains-${new Date().toLocaleDateString()}.txt`, {
    encoding: 'utf8',
  });
  ...
  while (domainArray.length > 0) {
    ...
    // 查询域名
    const data = JSON.parse(await query(theQueryArray)); // eslint-disable-line no-await-in-loop
    // 获取成功
